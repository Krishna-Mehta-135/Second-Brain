...
/**
 * Channel prefix for Pub/Sub. Combined with the keyPrefix from RedisOptions,
 * the full channel on Redis becomes `knowdex:crdt:updates:<docId>`.
 */
const REDIS_CHANNEL_PREFIX = "crdt:updates:";

export type RedisUpdateHandler = (
  update: Uint8Array,
...
export class RedisTransport {
  ...
  constructor(redisUrlOrConfig: string | RedisOptions, logger: Logger) {
    this.logger = logger;
    ...
    if (typeof redisUrlOrConfig === "string") {
      this.pub = new Redis(redisUrlOrConfig, commonConfig);
      this.sub = new Redis(redisUrlOrConfig, commonConfig);
    } else {
      const opts: RedisOptions = { ...commonConfig, ...redisUrlOrConfig };
  ...
  public publish(
    ...
    this.pub
      .publish(`${REDIS_CHANNEL_PREFIX}${docId}`, JSON.stringify(envelope))
      .catch((err) => {
  ...
  public async resubscribeAll(): Promise<void> {
    ...
    const channels = Array.from(this.activeDocIds).map(
      (id) => `${REDIS_CHANNEL_PREFIX}${id}`,
    );
    try {
