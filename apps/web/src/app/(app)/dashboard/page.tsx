...
function Dashboard() {
  ...
  const fetchContent = async () => {
    try {
      // Content fetching will need to be updated to use the session cookie
      // or we'll need a way to pass the token if the API is on a different domain.
      // For now, let's assume the API validates the session cookie.
  ...
  const handleShareBrain = async () => {
    ...
    try {
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:9898/api/v1"}/brain/share`,
        {
          isPublic: true,
        },
        {
  ...
  const handleDeleteContent = async (id: string) => {
    try {
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:9898/api/v1"}/content/${id}`,
        {
          withCredentials: true,
        },
      );
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center text-center py-20">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-md">
                Start building your second brain by saving articles, videos,
                tweets, and other valuable content.
              </p>
              <Button
                variant="Primary"
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                ...
                <strong>{search}</strong>&quot; or the selected filter.
              </p>
            </div>
          ) : (
